/*! Telesense v0.1.0 | MIT | https://github.com/EbParsa/Telesense */
/*!
...
'use strict';
const DEFAULTS={
endpoint: '',supabaseUrl: '',supabaseAnonKey: '',flushInterval: 5000,maxQueue: 500,maxStored: 20000,mouseThrottle: 100,sessionId: null,capture: {
click: true,mousemove: true,scroll: true,keydown: true,keyup: true,visibility: true,resize: true,},onFlush: null,onEvent: null,};
const uid=() => {
...
tag: el.tagName.toLowerCase(),id: el.id || null,classes: typeof el.className === 'string'
? el.className.split(/\s+/).filter(Boolean) : [],});
function createTele(userConfig={}) {
const cfg=Object.assign({}, DEFAULTS, userConfig,{ capture: Object.assign({}, DEFAULTS.capture, userConfig.capture) });
...
const event={
type,ts: Date.now(),sessionId,page: location.pathname,...data,};
queue.push(event);
...
fetch(cfg.endpoint, {
method: 'POST',headers:{'Content-Type': 'application/json' },body,keepalive: true,}).catch(() => persistLocal(events));
};
...
await fetch(url, {
method: 'POST',headers: {
'Content-Type': 'application/json',apikey: cfg.supabaseAnonKey,Authorization: `Bearer ${cfg.supabaseAnonKey}`,Prefer: 'return=minimal',},body: JSON.stringify(rows),keepalive: true,});
};
...
button: e.button === 2 ? 'right' : 'left',x: e.clientX,y: e.clientY,element: elSummary(e.target),});
handlers.contextmenu=(e) => record('click', {
button: 'right',x: e.clientX,y: e.clientY,element: elSummary(e.target),});
...
record('scroll', {
scrollX: window.scrollX,scrollY: window.scrollY,percentX: Math.round((window.scrollX / maxX) * 100),percentY: Math.round((window.scrollY / maxY) * 100),});
};
document.addEventListener('scroll', handlers.scroll,{passive: true });
}
if (cfg.capture.keydown) {
handlers.keydown=(e) => record('keydown', {
key: isSensitive(e.target) ? '[masked]' : e.key,code: e.code,target: isSensitive(e.target) ? '[sensitive]' : elSummary(e.target),});
...
record(type, data);
return api;
},flush: () => flush(false),getQueue: () => [...queue],reset() {
queue=[];
return api;
},config(patch={}) {
...
return api;
},start: attach,stop: detach,get sessionId(){return sessionId; },version: '1.0.0',};
